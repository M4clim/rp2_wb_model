...
def run_simulation(model, num_steps, num_mc_sweeps=1, num_relax_steps=5, callback=None):
    ...
    print(
        f"Paramètres: T_eff={model.config['T_eff']}, "
        f"DeltaE={model.config['DeltaE']}, "
        f"J={model.config['J_Wb_normal']}, "
        f"J'={model.config['J_prime_Wb_ts']}"
    )
    ...
    for step in range(num_steps):
        ...
        if step % max(1, num_steps // 20) == 0 or step == num_steps - 1:
            print(
                f"Étape {step:5d}: N_pot={stats['N_pot']:8.2f}, "
                f"Num_A={stats['num_active']:5d}, "
                f"Cohérence={stats['coherence']:.3f}, "
                f"Acc_Ratio={stats['acceptance_ratio']:.4f}"
            )

            # Vérification si N_pot est épuisé
            if (
                model.N_pot <= 1e-9
                and model.config["delta_Npot_base_actualisation"] > 0
            ):
...
def analyze_phase_structure(model):
    ...
    active_nodes = [
        node for node in range(model.num_nodes) if model.sigma.get(node, -1) == 1
    ]
