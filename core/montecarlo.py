...
def metropolis_step(model, node, current_energy=None, proposed_sigma=None):
    """
    Implémente l'algorithme de Metropolis pour décider si un flip de spin doit être effectué
    
    Args:
        model (WBModel): Instance du modèle WB
        node: Identifiant du nœud à mettre à jour
        current_energy (float, optional): Énergie actuelle du nœud (calculée si None)
        proposed_sigma (int, optional): Valeur proposée pour σ (inverse de l'actuelle si None)
        
    Returns:
        tuple: (did_flip, energy_delta, N_pot_after) - Si un flip a été effectué, le changement d'énergie et N_pot après
    """
    ...
    barrier_cost, potential_consumption = model.calculate_barrier_cost(node, proposed_sigma)
...
def monte_carlo_sweep(model, num_sweeps=1):
    ...
    acceptance_ratio = total_flips / (num_sweeps * num_nodes) if num_sweeps * num_nodes > 0 else 0
            ...
            if model.sigma.get(i, -1) == 1:
                ...
                for j in model.graph.get_neighbors(i):
                    ...
                    if model.sigma.get(j, -1) == 1:
                        ...
                        J_eff = model.config["J_prime_Wb_ts"] if is_ts else model.config["J_Wb_normal"]
                        A_top = np.pi if is_ts else 0.0
                        
                        # Force sur la phase
                        phi_force += -J_eff * np.sin(model.phi.get(i, 0) - model.phi.get(j, 0) - A_top)
