...
class WBModel:
    ...
    def __init__(self, graph, config=None):
        ...
        self.rho = {}  # Champ ρ (densité effective)
    ...
    def initialize_fields(
        self, sigma_config="all_P", phi_config="random", random_seed=None
    ):
        """
        ...
        # Initialisation de ρ (densité effective)
        for node in self.network.nodes():
            self.rho[node] = self.get_effective_rho(node)

        print(f"Champs initialisés pour {self.num_nodes} nœuds")
        print(
            f"État initial: N_pot={self.N_pot:.2f}, Num_A={self.count_active_nodes()}"
        )
    ...
    def get_node_state(self, node):
        ...
        return {
            ...
            "rho": self.rho.get(node, 0),  # ρ effectif calculé
        }
    ...
    def get_effective_rho(self, node):
        ...
        """
        return self._rho_single(node)

    def _rho_single(self, n):
        """
        Calcule la densité effective ρ pour un nœud basée sur ses voisins actifs

        Args:
            n: Identifiant du nœud

        Returns:
            float: Densité effective ρ
        """
        # Moyenne pondérée des σ voisins
        neigh = self.graph.get_neighbors(n)
        active = sum(1 for j in neigh if self.sigma[j] == 1)
        return 0.1 + 0.9 * active / max(1, len(neigh))
    ...
    def flip_spin(self, node):
        ...
        if node in self.sigma:
            self.sigma[node] *= -1
            self.rho[node] = self.get_effective_rho(node)  # Mettre à jour ρ
            return True
        return False
    ...
    def calculate_interaction_energy(self, node1, node2):
        ...
        if sigma_i == 1 and sigma_j == 1:
            ...
            J_eff = (
                self.config["J_prime_Wb_ts"] if is_ts else self.config["J_Wb_normal"]
            )
    ...
    def delta_energy_flip(self, node):
        """
        Calcule la différence d'énergie si on inverse le spin d'un nœud

        Args:
            node: Identifiant du nœud

        Returns:
            float: Différence d'énergie (après - avant)
        """
        before = 0.0
        after = 0.0
        ...
        for neighbor in self.graph.get_neighbors(node):
            ...
            # Déphasage topologique pour les liens TS
            phase_shift = np.pi if is_ts else 0.0

            # Calcul du terme de phase
            phase_term = np.cos(self.phi[node] - self.phi[neighbor] - phase_shift)

            # Énergie avant le flip
            if self.sigma[node] == 1 and self.sigma[neighbor] == 1:
                before += -self.config["J_Wb_normal"] * phase_term

            # Énergie après le flip (spin inversé)
            if -self.sigma[node] == 1 and self.sigma[neighbor] == 1:
                after += -self.config["J_Wb_normal"] * phase_term

        # Retourne la différence d'énergie
        return after - before
    ...
    def calculate_barrier_cost(self, node, proposed_sigma):
        ...
        Returns:
            tuple: (barrier_cost, potential_consumption)
        """
        current_sigma = self.sigma.get(node, 0)
        total_barrier = 0.0
        potential_N_pot_consumption = 0.0

        # Coût de base pour P→A
        if current_sigma == -1 and proposed_sigma == 1:
            base_cost = self.config["delta_Npot_base_actualisation"]
            potential_N_pot_consumption += base_cost

        # Parcours des voisins TS pour calculer les barrières supplémentaires
        for neighbor in self.graph.get_neighbors(node, kind="ts"):
            neighbor_sigma = self.sigma.get(neighbor, 0)
            phi_i = self.phi.get(node, 0)
            phi_j = self.phi.get(neighbor, 0)

            # Terme de phase pour la barrière
            phase_term = self.config["alpha_phase"] * (
                1.0 - np.cos(phi_i - phi_j - np.pi)
            )

            # Initialiser barrier à 0 par défaut
            barrier = 0.0
            ...
            if current_sigma == -1 and proposed_sigma == 1:  # P→A
                ...
                if neighbor_sigma == 1:
                    ...
                    barrier = (
                        self.config["V0_base"]
                        * (self.config["beta_asym"] + phase_term)
                        * F_Npot
                    )
    ...
    def can_afford_transition(self, node, proposed_sigma):
        ...
        if current_sigma == -1 and proposed_sigma == 1:
            ...
            return self.N_pot >= potential_consumption, potential_consumption
